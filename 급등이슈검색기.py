...
if uploaded_file:
    ...
    if keyword:
        ...
        # 같은 테마/급등이슈로 상승한 종목 모아서 비교
        if not filtered.empty:
            st.write("### 📊 같은 급등이슈로 상승한 종목 비교")
            for issue in filtered['급등이슈'].unique():
                same_issue_df = df[df['급등이슈'] == issue]
                if len(same_issue_df) > 1:
                    st.write(f"#### 🔸 {issue} ({len(same_issue_df)}종목)")
                    st.dataframe(same_issue_df, use_container_width=True)

        # 키워드 포함한 모든 급등이슈 출력
        st.write("### 📚 키워드가 포함된 전체 급등 이슈 리스트")
        keyword_df = df[df['급등이슈'].str.contains(keyword, case=False, na=False)]
